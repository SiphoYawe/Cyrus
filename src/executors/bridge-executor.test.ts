...
import type { BridgeExecutorConfig } from './bridge-executor.js';
...
function makeSwapAction(): SwapAction {
  return {
    id: 'swap-1',
    type: 'swap',
    priority: 5,
...
describe('BridgeExecutor', () => {
  ...
  describe('canHandle', () => {
    ...
    it('returns false for swap action', () => {
      ...
      expect(executor.canHandle(makeSwapAction())).toBe(false);
    });
  });
  ...
  describe('trigger stage', () => {
    it('rejects same-chain transfers', async () => {
      ...
      const result = await executor.execute(makeBridgeAction({ toChain: FROM_CHAIN }));
  ...
  describe('close stage', () => {
    it('handles COMPLETED status and records destination chain', async () => {
      setupBalance(store);
      const executor = new BridgeExecutor(
        createMockConnector(), createMockApprovalHandler(),
        createMockTransactionExecutor(), createMockPreFlightChecker(),
    ...
    it('handles REFUNDED status — restores source chain balance', async () => {
      setupBalance(store);
      const mockConnector = createMockConnector({
        getStatus: vi.fn().mockResolvedValue({
  ...
  describe('stage tracking', () => {
    it('tracks current stage through execution', async () => {
      ...
      await executor.execute(makeBridgeAction());
